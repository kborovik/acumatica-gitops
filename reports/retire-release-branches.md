...
Stop using `acu-25r1` / `acu-25r2` / `acu-26r1` as **product lines**.
Multi-version = trunk seed + multi-cell `matrix.yaml` + overlays.

## Preconditions (gate before delete)

1. Trunk (`main`) documents matrix model (T34–T35).
...
# 3. Optional: archive and delete former trunk name if it still exists
# git tag archive/acu-26r1 origin/acu-26r1 && git push origin archive/acu-26r1
# git push origin --delete acu-26r1

# 4. Optional local cleanup
git branch -D acu-25r1 acu-25r2 acu-26r1 2>/dev/null || true
```
...
| Branch | Status |
|--------|--------|
| `main` | **Trunk** (lab cells in `matrix.yaml`; default cell `26r1`) |
| `acu-26r1` | **Retired as product line** — delete remote alias if still present |
